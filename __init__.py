...
class Command:
    def __init__(self):#NOTE: init
        ...
        self.show_wo_alt= app.ini_read(fn_config, 'op', 'intextbookmk_compact_show', '1')=='1'
        self.unlxr_cmnt = app.ini_read(fn_config, 'op', 'intextbookmk_no_lexer_comment', '//')
        self.bm_signs   = app.ini_read(fn_config, 'op', 'intextbookmk_signs', ' '.join(['NOTE:', 'NB!', 'TODO:', 'todo:', 'todo.', 'FIX:'])).split(' ')
        ...
        self.ext2lxr    = {}
        self.find_indent = int(app.ini_read(fn_config, 'op', 'find_indent_vert', '2')) #AT
...
#       if lxr not in self.lxr2cmnt:    return app.msg_status(f(_('Cannot add in-text bookmark into document with Lexer {}. No to-end-of-line comment.'), lxr))
    ...
    def _jump_to_ibm(self, what):
        ...
        if not (ed.get_prop(app.PROP_LINE_TOP) <= line_n <= ed.get_prop(app.PROP_LINE_BOTTOM)):
            ed.set_prop(app.PROP_LINE_TOP, str(max(0, line_n - max(5, self.find_indent))))
    ...
    def dlg_ibms_in_tab(self):
        ...
        if self.show_wo_alt:
            ans = app.dlg_menu(app.MENU_SIMPLE, 'List', '\n'.join(
                    [f('{}\t{}', line_nd, bm_msg) for bm_msg, line_n, line_nd in ibms]
                ), near)
        else:
            ans = app.dlg_menu(app.MENU_DOUBLE, 'List', '\n'.join(
        ...
        while True:
            ...
            if act_cid=='!':
                ...
                if  self.bm_signs  != vals['sgns'].split():
                    ...
                    app.ini_write(fn_config, 'op', 'intextbookmk_signs', ' '.join(self.bm_signs))
                if  self.unlxr_cmnt!= vals['dfcm'].strip():
                    self.unlxr_cmnt = vals['dfcm'].strip()
                    app.ini_write(fn_config, 'op', 'intextbookmk_no_lexer_comment', self.unlxr_cmnt)
                if  self.wrap      != vals['wrap']:
                    self.wrap       = vals['wrap']
                    app.ini_write(fn_config, 'op', 'intextbookmk_wrap', '1' if self.wrap else '0')
                break#while
           #while
       #def dlg_config
